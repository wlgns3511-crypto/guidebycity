...
function getAffordabilityLevel(score: number): { label: string; color: string; ringColor: string; bg: string } {
  ...
  return { label: "High Cost Burden", color: "text-rose-700", ringColor: "stroke-rose-500", bg: "bg-rose-50" };
}

function getResilienceLevel(score: number): { label: string; color: string; ringColor: string; bg: string } {
  if (score >= 80) {
    return { label: "High Resilience", color: "text-teal-700", ringColor: "stroke-teal-500", bg: "bg-teal-50" };
  }
  if (score >= 50) {
    return { label: "Moderate Risk", color: "text-amber-700", ringColor: "stroke-amber-500", bg: "bg-amber-50" };
  }
  return { label: "High Hazard Risk", color: "text-rose-700", ringColor: "stroke-rose-500", bg: "bg-rose-50" };
}
...
export function ProprietaryMetricsBlock({
  ...
  const aff = getAffordabilityLevel(housingAffordabilityScore);
  const res = getResilienceLevel(environmentalResilienceScore);
  const gradeStyles = getGradeStyles(overallGrade);

  // SVG Circle parameters for progress gauge
  const radius = 36;
  const circumference = 2 * Math.PI * radius;
  const affDashoffset = circumference - (housingAffordabilityScore / 100) * circumference;
  const resDashoffset = circumference - (environmentalResilienceScore / 100) * circumference;

  return (
    <section
      data-upgrade="proprietary-metrics"
      aria-label="GuideByCity Proprietary Relocation and Livability Ratings"
    ...
    >
      ...
      <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="flex flex-col items-center">
            <div className="relative w-24 h-24 flex items-center justify-center">
              ...
              <div className="absolute flex flex-col items-center justify-center">
                <span className="text-2xl font-black text-slate-800">{housingAffordabilityScore}</span>
          ...
          <div className="flex flex-col items-center">
            <div className="relative w-24 h-24 flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 96 96">
                ...
                {/* Active Ring */}
                <circle
                  className={`${res.ringColor} transition-all duration-500`}
                  strokeWidth="8"
                  strokeDasharray={circumference}
                  strokeDashoffset={resDashoffset}
                  strokeLinecap="round"
                  stroke="currentColor"
                  fill="transparent"
                  r={radius}
              ...
              {/* Score Text */}
              <div className="absolute flex flex-col items-center justify-center">
                <span className="text-2xl font-black text-slate-800">{environmentalResilienceScore}</span>
                <span className="text-[8px] text-slate-400 font-semibold uppercase tracking-wider text-center max-w-[50px] leading-tight font-sans">Resilience</span>
              </div>
            </div>
            <span className={`text-xs font-bold mt-2 ${res.color}`}>{res.label}</span>
          </div>

          {/* Relocation Grade Badge */}
          <div className="flex flex-col items-center">
        ...
        <div className="flex-1 bg-teal-50/20 border border-teal-100/50 rounded-xl p-4.5">
          <h4 className="text-xs font-bold text-teal-600 uppercase tracking-wider mb-1 font-sans">Relocation Expert Analysis</h4>
      ...
      <div className="mt-4 border-t border-slate-100 pt-3 flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-slate-400">
        <span>* Affordability Score measures average purchasing power and price-to-income balance against the national Demographia standard.</span>
        <span>* Resilience Score measures environmental risk safety and natural disaster readiness based on FEMA national risk data.</span>
        <span>* Overall Grade is a composite of Affordability, Resilience, and Economic Vitality (based on trailing population trends).</span>
      </div>
    </section>
  );
}
